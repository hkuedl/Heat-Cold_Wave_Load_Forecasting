...
def coldwave_dataloader(country='Belgium', coldwave_samples=None):
    ...
    #coldwave_samples = generate_hotwave_samples(country, num_samples=1300).cpu().detach().numpy()
...
def test_dataloader(country='Belgium', strat_time = '2023/05/01/00', end_time = '2023/09/30/23'):
    seed = 2
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    ...
    load_slice_list, tem_slice_list, weekday_index_list, \
        coldwave_index, hotwave_index = clear_diff_data(country, strat_time = strat_time, end_time = end_time)
...
def results_visualization_impactnet(country='Belgium', data_aug=0, model_type='impactnet'):
    ...
    ax.plot(forecasted_value[24 * day:24 * (day + period)], label='forecasted')
    ax.plot(true_value[24 * day:24 * (day + period)], label='true')
    ax.fill_between([192 + 120 + i for i in range(96)],  # x值范围
                    [0 for i in range(96)],  # 下边界（曲线本身）
                    [1.0 for i in range(96)],  # 上边界（比曲线最高点高2个单位）
                    color='salmon',
                    alpha=0.1)
    # plt.plot(true_tem[24*(day):24*(day+60)], label='tem')
    ax.set_xlabel('Time index [h]')
    ax.set_ylabel('Norm. Load')
    ax.set_title('2023 PJM Heatwave')
...
#test_country_list = ['American Electric Power Co., Inc']
...
for test_country in test_country_list:
    ...
    torch.backends.cudnn.benchmark = False
    os.environ['PYTHONHASHSEED'] = str(seed)
    coldwave_samples = generate_hotwave_samples(test_country, num_samples=int(800*proportion)).cpu().detach().numpy()
